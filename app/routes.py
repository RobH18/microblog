...
def index():
    ...
    if posts.has_prev:
        ...
        prev_url = None
    return render_template('index.html', title='Home', form=form, posts=posts.items, next_url=next_url, prev_url=prev_url) # .items is because of paginate
...
def reset_password_request():
    ...
    if form.validate_on_submit():
        ...
        flash('Check your email for the instructions to reset your password') # flashes regardless, so users cannot find if a user exists or not
        return redirect(url_for('login'))
